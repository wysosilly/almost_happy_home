...
public enum FurnitureType { Normal, Storage }
...
public class Furniture : MonoBehaviour
{
    ...
    void SetupNameLabel() {
        string nameText = string.IsNullOrEmpty(displayName) ? gameObject.name : displayName;
    ...
    public Vector3 GetWorldPosition() {
        ...
        var s = EffectiveSize;
        float x = pivotAtCenter ? gridPos.x + (s.x * 0.5f) : gridPos.x;
        float z = pivotAtCenter ? gridPos.y + (s.y * 0.5f) : gridPos.y;
    ...
    public List<Vector2Int> GetOccupiedCellsAt(Vector2Int pos) {
        ...
        if (useCustomShape && customShapeCells.Count > 0) {
            foreach (var offset in GetRotatedShapeCells())
                cells.Add(new Vector2Int(pos.x + offset.x, pos.y + offset.y));
        } else {
            var s = EffectiveSize;
            for (int x = 0; x < s.x; x++)
                for (int y = 0; y < s.y; y++)
